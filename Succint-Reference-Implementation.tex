...
Similarly, we can forget operations $S$ from a state $(O,\mathit{ro},\mathit{del},\mathit{vis})$ by delete $S$ from $O$ and projection the relations to non-$S$ operations. Given a state $q = (O,\mathit{ro},\mathit{del},\mathit{vis})$, its succinct version $suct(q)$ is obtained from $q$ by forgetting $RdtA(O,\mathit{vis}) \cup \{ o_r \vert  \{ o_a \vert o_r \in f_R(O,\mathit{vis},o_a) \} \subseteq RdtA(O,\mathit{vis}) \}$. %(Here we assume isomorphism-closed of operation identifiers).

Given a collection specification $Spec$, its succinct reference implementation is given as an LTS $SRImp(Spec) = (Q_s,\Sigma,q_0,\rightarrow_s)$, which can be obtained from $RImp(Spec) = (Q,\Sigma,q_0,\rightarrow)$ as follows: %use the transition rules of $\rightarrow$, and after every step we remove the redundant operations. Formally,
