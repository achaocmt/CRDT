...
\begin{remark}
  ...
  %
  \fxnote*{GP: @Chao, can you rephrase this one?}{\color {red}When we extract $<$ from $\mathit{vis}$, such pairs $(o_1,o_2)$ will be ignored: $o_2$ does not cancel $o_1$ ($o_2 \notin \mathit{FstRem}(o_1)$), and some intermediate operations between $o_1$ and $o_2$ w.r.t $\mathit{vis}$ is not in $O$.
