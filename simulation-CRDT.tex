...
\item[-] For non-redundant operations, a condition of $f$ maps which REMOVE to be the corresponding REMOVE of which ADD. 

\item[-] For non-redundant $o$, $(o,r) \in msgs$, if and only if $f(o) \notin visTo(q_s,r)$.

\item[-] When $q_{\mathit{imp}}$ is in certain form, an ADD operation is seen to replica $r$, and there exists one of its corresponding REMOVE seen to replica $r$. 
\end{itemize}
...
The $f_s$-simulation relation is defined as follows: If $(q_{\mathit{imp}},q_s) \in R_{\mathit{sim}} \wedge q_{\mathit{imp}} {\xrightarrow{\alpha}}_{\mathit{imp}} q'_{\mathit{imp}}$ and let $f$ be the map between $q_{\mathit{imp}}$ and $q_s$, then, let $f_s$ be defined as follows:
