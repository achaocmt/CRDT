...
\item[-] {\color {red}Let $ctxt$ be a function that takes $(O,ro,del,arb) \in Q$ and $o=(\_,r,\_) \notin O$ as arguments, and returns a tuple $(O_1,<_1,arb_1)$, where 

    \begin{itemize}
    \setlength{\itemsep}{0.5pt}
    \item[-] $O_1 = visTo(O,r,vis)$. Here $visTo(O,r,vis) = \{ vis^{-1}(o') \vert o'=(\_,r,\_) \in O \} \cup (vis^{-1}(r))$.


    \item[-] $<_1 \subseteq vis \uparrow_{( O_1 \times O_1 )}$. 

    \item[-] $arb_1$ is obtained from $arb$ by projection into $visTo(O,r,vis) \times visTo(O,r,vis)$, and then possibly  add operation $o$ and some relation between $O_1$ and $\{ o \}$.
    \end{itemize}}
