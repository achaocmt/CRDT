...
Given a specification $Spec$, its reference implementation is given as an tuple $RImp(Spec) = (Q,\Sigma,q_0,\rightarrow)$, where
...
\item[-] Each state $(O,ro,del,arb) \in Q$ contains five tuples. Here $O$ is a set of update operations, and we assume that no two operations in $O$ have same operation identifier. $ro$ is the replica order over operations in $O$. $del \subseteq (O \times O) \cup (O \times \mathbb{R})$ is the deliver order. $(o_1,o_2) \in del$ represents that the effect of $o_1$ is delivered to the replica of $o_2$ before $o_2$ happens, while $(o,r) \in del$ represents that the effect of $o$ is delivered to replica $r$ after the time point of the last operation of replica $r$. we require $del$ to only relate operations with different replica identifier. $arb \subseteq O \times O$ is the arbitration order.

    Let $vis = del \cdot ro$ be the visibility relation. %$(o_1,o_2) \in vis$ represents that the effect of $o_1$ is visible to $o_2$, while $(o,r)$ represents that the effect of $o$ becomes visible to replica $r$ after the time point of the last operation of replica $r$.
    We require $vis$ to be acyclic and irreflexive, and $vis^{-1}$ to be finite.

%\todo{$vis$ shouldn't be in the state.}

\item[-] $\Sigma = \Sigma_1 \cup \Sigma_2$ is the set of transition labels, where $\Sigma_1 = \{ (m,a,b,r,arb) \vert m \in \mathbb{M}, a,b \in \mathbb{D}, r \in \mathbb{R},$ $arb \subseteq O \times O\}$ and $\Sigma_2 = \{ addDel(o,r) \vert o \in \mathbb{O}, r \in \mathbb{R} \}$.

\item[-] {\color {red}Let $ctxt$ be a function that takes $(O,ro,del,arb) \in Q$ and $o=(\_,r,\_) \notin O$ as arguments, and returns a set of operation contexts. Each element in this set is a operation context of $o$ for some annotated history $(O_1,ro_1,vis_1,arb_1)$ satisfies that

    \begin{itemize}
    \setlength{\itemsep}{0.5pt}
    \item[-] $O_1 = visTo(r) \cup \{ o \}$. Here $visTo(O,r) = \{ vis^{-1}(o') \vert o'=(\_,r,\_) \in O \} \cup (vis^{-1}(r))$.


    \item[-] $ro_1 = ro \cup \{ (o',o) \vert o'=(\_,r,\_) \in O \}$.

    \item[-] $vis_1 = (vis \uparrow_{O \times O}) \cup \{ (o',o) \vert (o',r) \in vis \}$.

    \item[-] $arb_1 \subseteq O_1 \times O_1$ is obtained from $arb$ by projection into $visTo(r)$, and then add operation $o$ and possibly some relation between $O$ and $\{ o \}$.
    \end{itemize}}

\item[-] $\rightarrow \subseteq Q \times \Sigma \times Q$ is the transition relation. {\color {red} We assume there exist a predicate $cor: Q \rightarrow \{ \mathit{true},\mathit{false} \}$, and require that if there is a transition between a pair of stats, then they both satisfy $cor$.}
%It contains three kinds of transitions: adding a deliver relation, doing a query operation, and doing an update operation.

    \begin {itemize}
    \item[-] Adding a deliver relation:

     $\begin{array}{l c} \bigfrac{o \in O, \neg(o \xrightarrow{vis} r)} {(O,ro,del,arb) {\xrightarrow{addDel(o,r)}} (O,ro,del \cup \{ (o,r) \},arb)} \end{array}$

    \item[-] Doing a query operation: Let $m$ be a query method. For $\ell = m(a) \Rightarrow b$,

     $\begin{array}{l c} \bigfrac{ o=(\ell,r,\_) \notin O, \exists O', x= (O',\_,arb \uparrow_{ (Q' \times Q') }) \in ctxt((O,ro,del,arb),o), x \in Spec(\ell)} {(O,ro,del,arb) {\xrightarrow{m(a,b,r,arb)}} (O,ro,del,arb)}  \end{array}$

     \item[-] Doing an update operation: Let $m$ be an update method, let $arb'$ be obtained from $arb$ by adding $o$ and possibly relations between $O$ and $\{ o \}$. Let $arb' \uparrow_{(O' \cup \{ o \})}$ short for $arb' \uparrow_{(O' \cup \{ o \}) \times (O' \cup \{ o \})}$. For $\ell = m(a) \Rightarrow b$,

     $\begin{array}{l c} \bigfrac{o = (\ell,r,\_) \notin O, \exists O', x = (O',\_,arb' \uparrow_{(O' \cup \{ o \})}) \in ctxt((O,ro,del,arb),o), x \in Spec(\ell)} {(O,ro,del,arb) {\xrightarrow{m(a,b,r,arb')}} (O \cup \{ o \},ro \oplus o ,del \oplus o,arb')}  \end{array}$

     $ro \oplus o = ro \cup \{ (o',o) \vert o' = (\_,r,\_) \in O \}$, and $del \oplus o$ is obtained from $del$ by transforming each $(o',r)$ into $(o',o)$. We further require that the candidate of operation identifier of $o$ is unique.
    \end{itemize}
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Given $o=(\_,r,\_)$ and $q=(O,ro,del,\emptyset)$, $ctxt(q,o) = (visTo(O,r),<_1,\emptyset)$, where $<_1 = vis \uparrow_{(O_1 \times O_1)} - \{ (o_1,o_2) \vert o_2 \notin Minus(q,o_1), \exists o_3, o_1 {\xrightarrow{vis}} o_3 {\xrightarrow{vis}} o_2, o_1,o_2 \in visTo(O,r), o_3 \notin visTo(O,r) \}$.
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Given $q = (O,ro,del,arb)$ and $o =(\_,r,\_)$, $ctxt(q,o) = ( O_1 ,<_1, arb_1 )$, where $O_1 = visTo(O,r)$, $<_1 = vis \uparrow_{(O_1 \times O_1)}$, and $arb_1$ is as follows: (1) if $lab(o)=add(\_)$, then $arb_1$ is a total order generated by adding $o$ into some place of $arb$, (2) otherwise, $arb_1 = arb$.
...
It is obvious that $RImp(Spec)$ is deterministic. Let $\llbracket RImp(Spec) \rrbracket = \{ \alpha_1 \cdot \ldots  \vert \exists q_1,\ldots \in Q, q_0 {\xrightarrow{\alpha_1}} q_1 \ldots \}$. Given an sequence $ t= \alpha_1 \cdots \in \llbracket RImp(Spec) \rrbracket$, its annotated history $anHis(t) = (O,\mathit{ro},\mathit{vis},\mathit{arb})$ is generated as follows: Let $e = q_0 {\xrightarrow{\alpha_1}} q_1 \ldots$ be an execution,

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $O$ is the set of operations generated during $e$.

\item[-] $(o_1,o_2) \in \mathit{ro}$, if they have the same replica identifier, $o_1$ (resp., $o_2$) is generated in transition $\alpha_u$ (resp., $\alpha_v$), and $u < v$.

\item[-] $(o_1,o_2) \in \mathit{vis}$, if one of the following case holds: (1) $o_1$ is delivered into replica of $o_2$, and then $o_2$ happens, (2) $o_1$ and $o_2$ are of same replica and $o_1$ happens earlier than $o_2$.

\item[-] $<_{\mathit{arb}}$ is the union of arbitration order of each $q_i$.
\end{itemize}

The following theorem states that the traces of $RImp(Spec)$ are SRVC w.r.t $Spec$. Its proof can be found in Appendix \ref{sec:appendix definitions and proofs of section reference implementation}.

\begin{theorem}
\label{lemma:executions of reference implementation are SRV consistent}
$\forall t \in \llbracket RImp(Spec) \rrbracket$, $anHis(t)$ is SRVC consistent w.r.t $Spec$.
\end{theorem}


\noindent {\bf Reference Implementation with Causal Delivery}:

An operation $o_1$ happen-before \cite{Lamport:1978} an operation $o_2$, denoted $o_1 <_{hb} o_2$, if $o_1 {\xrightarrow{ (del \cup ro)^* }} o_2$. A trace satisfies causal delivery, if for each pair of update operations $(o_1,o_2)$ of this trace, if $o_1 <_{hb} o_2$, then for each replica $r$, $o_2$ can be delivered to replica $r$ if $o_1$ has already been delivered to replica $r$. 

The reference implementation with causal delivery of $Spec$ is given as an tuple $RImp(Spec)_{\mathit{cd}} = (Q,\Sigma,q_0,\rightarrow)$. $RImp_{\mathit{cd}}(Spec)$ can be obtained from $RImp(Spec)$ by only changing the transition rules of adding a delivery relation as follows:

\begin {itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Adding a deliver relation:

$\begin{array}{l c} \bigfrac{o \in O, o \ is \ minimal \ w.r.t \ <_{hb} in \ \{ o' \vert o' \in O \wedge o' \notin visTo(O,r) \}} {(O,ro,del,arb) {\xrightarrow{addDel(o,r)}} (O,ro,del \cup \{ (o,r) \},arb)} \end{array}$
...
OR-set is a plus-minus specification, while $add$ is the ``plus method'', $rem$ is the ``minus method'', and $lookup$ and $elements$ are query methods.

Distributed list is a plus-minus specification, while $add$ is the ``plus method'', $rem$ is the ``minus method'', and $read$ is query method. %Here we assume that for $add$ and $rem$, $pos$ is less than the length of list seen by calling replica.
...
Given a plus-minus specification $Spec$ and a state $q=(O,ro,del,arb,vis)$ of $RImp(Spec)$, given a plus operation $o \in O$, let $Minus(q,o)$ be a set of minus operations of $O$. Let us define $conc(q)$:
...
\item[-] We say $q' = conc(q)$, if: Let $q''$ be obtained from $q$ by penetrating operations in $S_p(q) \cup S_m(q)$ one by one, and there is a bijection between operations of $q'$ and $q''$ that keeps the operation identifiers, operation labels and $ro$, $del$ and $arb$ relations.
\end{itemize}

We require our definition of $Minus$ to satisfy the following properties: We say $q' = pent(q,O)$ if there exists $q''$, such that $q''$ is obtained from $q$ by penetrating operations in $S_p(q) \cup S_m(q)$ one by one, and there is a bijection between operations of $q'$ and $q''$ that keeps the operation identifiers, operation labels and $ro$, $del$ and $arb$ relations. $MatchRedunt(q,O)$ holds, if $\exists O_1$, such that $O_1 \subseteq S_p(q) \wedge O = O_1 \cup \{ o_1 \vert  \{ o_2 \vert o_1 \in Minus(o_2) \} \subseteq O_1 \}$. Given set $O$, relation $R_{O}$ is defined as follows: $(q,q') \in R_{O}$, if $q' = pent(q,O) \wedge MatchRedunt(q,O)$. Let $map_{(q,q',O)}$ the projection which are used to check $q' = pent(q,O)$. We further require $R_{O}$ to satisfy that,
...
\item[-] given $(q_i,q'_i) \in R_{O}$,
    ...
    \begin{itemize}
    \setlength{\itemsep}{0.5pt}
    %\item[-] If $q_i {\xrightarrow{addDel(o,r)}} q_{i+1} \wedge o \in S_p(q_i) \cup S_m(q_i)$, then $(q_{i+1},q') \in R_{O}$.

    %\item[-] If $q_i {\xrightarrow{\alpha}} q_{i+1}$ and $\neg( \alpha = addDel(o,r) \wedge o \in O )$, then $q'_i {\xrightarrow{\alpha'}} q'_{i+1} \wedge (q_{i+1},q'_{i+1}) \in R_{O}$, where

        %\begin{itemize}
        %\setlength{\itemsep}{0.5pt}
        %\item[-] If $\alpha = addDel(\_,\_)$, then $\alpha' = \alpha$,

        %\item[-] Else, if $\alpha = (m,a,b,r,arb')$, then $\alpha' = (m,a,b,r,arb-O)$.
        %\end{itemize}
...
%\item[-] $q_4 {\xrightarrow{\alpha_5}}_c q_5$: By definition of $\rightarrow_c$, $\exists \alpha'_5, q'_5$, such that $q_4 {\xrightarrow{\alpha'_5}}_r q'_5 \wedge conc(q'_5)=q_5$. Then, it is obvious that $(q'_5,q_5) \in R_{O_5}$ for some $O_5$.

%Since $(q''_4,q_4) \in R_{(O_2 \cup O_3 \cup O_4)} \wedge q_4 {\xrightarrow{\alpha'_5}}_r q'_5$, we can see that $\exists \alpha''_5,q''_5$, such that $q''_4 {\xrightarrow{\alpha''_5}}_r q''_5 \wedge (q''_5,q'_5) \in R_{(O_2 \cup O_3 \cup O_4)}$. Therefore, $(q''_5,q_5) \in R_{(O_2 \cup O_3 \cup O_4 \cup O_5)}$.
