...
\item[-] {\color {red}Let $ctxt$ be a function that takes $(O,ro,del,arb) \in Q$ and $o=(\_,r,\_) \notin O$ as arguments, and returns a operation context of $o$ in an annotated history $(O_1,ro_1,vis_1,arb_1)$, where .
    
    \begin{itemize}
    \setlength{\itemsep}{0.5pt}
    \item[-] $O_1 = \{ vis^{-1}(o') \vert o'=(\_,r,\_) \in O \} \cup (vis^{-1}(r)) \cup \{ o \}$. 

    \item[-] $ro_1 = ro \cup \{ (o',o) \vert o'=(\_,r,\_) \in O \}$. 
    
    \item[-] $vis_1 = (vis \uparrow_{O \times O}) \cup \{ (o',o) \vert (o',r) \in vis \}$. 
    
    \item[-] $arb_1 \subseteq O_1 \times O_1$ is obtained from $arb$ by projection into $O_1$ and then possibly adding some relation between $O$ and $\{ o \}$. 
    \end{itemize}}
...
%It contains three kinds of transitions: adding a deliver relation, doing a query operation, and doing an update operation. 
    ...
    \item[-] Doing a query operation: Let $m$ be a query method. For $\ell = m(a) \Rightarrow b$ and $q = (O,ro,del,arb)$, 

     $\begin{array}{l c} \bigfrac{ o=(\ell,r,\_) \notin O, ctxt(q,o) = (O',\_,arb \uparrow_{ (Q' \times Q') }) \in Spec(\ell)} {(O,ro,del,arb) {\xrightarrow{m(a,b,r,arb)}} (O,ro,del,arb)}  \end{array}$
...
\noindent {\bf Example 3. $RImp(S_{\mathit{ORS}})$}: $li$ and $correct$ is defined as follows:

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Given $q = (O,ro,del,arb,vis)$, $r \in \mathbb{R}$ and $o \notin O$, $li(q,r,o) = ( O_l ,<_l, <_{\mathit{l-arb}} )$. Here $<_{\mathit{l-arb}} = \emptyset$.
...
\noindent {\bf Example 4. $RImp(S_{\mathit{list}})$}: $li$ and $correct$ is defined as follows:

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Given $q = (O,ro,del,arb,vis)$, $r \in \mathbb{R}$ and $o \notin O$, $li(q,r,o) = ( O_l ,<_l, <_{\mathit{l-arb}} )$. Here $<_l = vis \uparrow_{(O_l \times O_l)}$. $<_{\mathit{l-arb}}$ is as follows: (1) if $o$ is an $add$ operation, then $<_{\mathit{l-arb}}$ is a total generated by adding $o$ into some place of $arb$, (2) otherwise, $<_{\mathit{l-arb}} = arb$.
...
Given an execution $t = \alpha_1 \cdots \in \llbracket RImp(Spec) \rrbracket$, its abstract trace $absT(t) = (O,<_{\mathit{ro}},<_{\mathit{vis}},<_{\mathit{arb}})$ is generated as follows: Let $q_1,\ldots \in Q$ be the set of states, such that $q_0 {\xrightarrow{\alpha_1}} q_1 \ldots$.

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $O$ is the set of operations generated during $t$.

\item[-] $<_{\mathit{ro}} = <_{r_1} \cup \ldots \cup <_{r_{\vert \mathbb{R} \vert}}$, where for each $i$, $<_i$ is the occurring order of operations with the $i$-th identifier in $\mathbb{R}$ over $t$.

\item[-] $(o_1,o_2) \in <_{\mathit{vis}}$, if one of the following case holds: (1) $o_1$ is delivered into replica of $o_2$, and then $o_2$ happens, (2) $o_1$ and $o_2$ are of same replica and $o_1$ happens earlier than $o_2$.
...
Given a specification $Spec$, its reference implementation with causal delivery is given as an tuple $RImp(Spec)_{\mathit{cd}} = (Q,\Sigma,q_0,\rightarrow,li,cor)$. $RImp_{\mathit{cd}}(Spec)$ can be obtained from $RImp(Spec)$ by only changing the transition rules of adding a delivery relation as follows:
...
The following lemma states that $RImp(Spec)_{\mathit{cd}}$ contains all the sequences of $RImp(Spec)$ that are causal delivery. Its proof can be found in Appendix \ref{sec:appendix definitions and proofs of section reference implementation}.
