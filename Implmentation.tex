...
Let $\Sigma_{\mathtt{RD}}$ be the set of data of each replica, and $\mathbb{MSG}$ be the set of messages. Each message is of the form $(data,r_s,r_d)$ where $r_s \in \mathbb{R}$ is the identifier of replica that generates this message, and $r_d \in \mathbb{R}$ is the destination replica identifier of this message. 

A CRDT implementation $imp$ is defined as a tuple $(f_M,f_{apl},init,f_{abO},f_{arb})$, where

\begin{itemize}
\setlength{\itemsep}{0.5pt} 
\item[-] $f_M: \Sigma_{\mathtt{RD}} \times \mathbb{M} \times \mathbb{D} \times \mathbb{R} \rightarrow \Sigma_{\mathtt{RD}} \times \mathbb{D} \times Pow(\mathbb{MSG})$ shows how method update data and generate messages. $f_M(d,m,a,r) = (d',b,msgs)$ means that, when the data of replica $r$ is $d$, if we call method $m$ with argument $a$ in replica $r$, then we obtain return value $b$, change the data of replica $r$ into $d'$, and generate a set $msgs$ of messages. $Pow(S)$ means the power set of $S$. Moreover, if $m$ is an update method, then $msgs = \{ (data,r,r') \vert r' \in \mathbb{R} \wedge r \neq r' \}$ for some $data$. Otherwise, $msgs = \emptyset$. 

\item[-] $f_{apl}: \Sigma_{\mathtt{RD}} \times \mathbb{R} \times \mathbb{MSG} \rightarrow \Sigma_{\mathtt{RD}}$. $f_{apl}(d,r,msg) = d'$ means that, if the current replica data of replica $r$ is $d$ and we apply message $msg$, then we obtain replica data $d'$.

\item[-] $init \in \Sigma_{\mathtt{RD}}$ is the initial replica data. 

\item[-] $f_{abO}: \Sigma_{\mathtt{RD}}^{\vert \mathbb{R} \vert} \times Pow(\mathbb{MSG}) \Rightarrow Pow(\Sigma_{abO})$, where $\Sigma_{abO}$ is a set of elements and each element represents an operation. Given replica data of each replica, $f_{abO}$ is used to obtain the operations known by implementation.

\item[-] $f_{arb}: \Sigma_{\mathtt{RD}}^{\vert \mathbb{R} \vert} \times Pow(\mathbb{MSG}) \Rightarrow \Sigma_{abO}^*$. Given replica data of each replica, $f_{arb}$ is used to obtain the arbitration relation.
\end{itemize} 

\noindent {\bf Example 5. OR-set algorithms of \cite{Shapiro:2011}}: The OR-set algorithms of \cite{Shapiro:2011} is defined as a tuple $(f_M,f_{apl},init,f_{abO},f_{arb})$, where 

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Each element of $\Sigma_{\mathtt{RD}}$ is a tuple $(S,ctr)$, where $S \subseteq \{ (a,id) \}$, $id$ contains replica identifier and a number, and $ctr$ is used to generate the next identifier. 

\item[-] $f_M((S,ctr),add,a,r) = ((S',ctr+1),msgs)$, where $S'=S \cup \{ (a,(r,ctr)) \}$, and $msgs = \{ ((a,(r,ctr)),r,r') \vert r' \neq r \}$. 

\item[-] $f_M((S,ctr),rem,a,r) = ((S',ctr),msgs)$, where $S'=S \setminus \{ (a,id) \in S \}$, $msgs = \{ ((a,S''),r,r') \vert r' \neq r \} $ where $S'' = \{ id \vert (a,id) \in S \}$. 

\item[-] $f_{apl}((S,ctr),r,((a,id),\_,r)) = (S \cup \{ (a,id) \},ctr)$. 

\item[-] $f_{apl}((S,ctr),r,((a,S'),\_,r)) = (S \setminus \{ (a,id) \vert id \in S' \},ctr)$. 

\item[-] $init = (\emptyset,0)$. 

\item[-] $f_{abO}( d_1,\ldots,d_k,msgs ) = \{ (a,id) \in d_1,\ldots,d_k \} \cup \{ (a,S) \in msgs \}$. 
\end{itemize} 
