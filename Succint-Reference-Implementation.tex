...
\item[-] Given a set $O$ of update operations and an relation $<$ over $O$, we can define the following notations: 
    ...
    \begin{itemize}
    \setlength{\itemsep}{0.5pt}
    \item[-] Given a ADD operation $o_a \in O$, the corresponding REMOVE operations of $a$ is given by a function $f_R(O,<,o_a)$. 
...
\item[-] Given an operation context $x = (O,<,\mathit{arb})$ and $S \subseteq O$, let $Forg(x,S)$ be a operation context $(O',<',\mathit{arb}')$ obtained from $x$ by forgetting $S$: $O' = O \setminus S$, $<' = < \uparrow_{(O' \times O')}$, and $\mathit{arb}' = \mathit{arb} \uparrow_{(O' \times O')}$. 

\item[-] An extension of a operation context $(O,<,\mathit{arb})$ is another operation context $(O',<',\mathit{arb}')$, such that $O \subseteq O'$, $\mathit{arb}$ is the projection of $\mathit{arb}'$ over $O$, $< \subseteq <'$, and $<' \cap ( (O'-O) \times O ) = \emptyset$. 
