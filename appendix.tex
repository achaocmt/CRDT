...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $P_1$: $msgs_i = \{ (dat,r,r') \vert$ the update operation $op(dat)$ is launched by replica $r$ and its message has not been applied to replica $r'$ in $t[1,i]\}$.

\item[-] $P_2$: $m_1 <_i m_2$, iff  $m_1 = (d_1,r_1,r')$, $m_2 = (d_2,r_2,r')$, and $op(d_1)$ happen before $op(d_2)$ in $t[1,i]$.

\item[-] $P_3$: $m <_i r$, iff $m=(d,r_1,r_2)$, $op(d)$ is visible to replica $r$ in $t[1,i]$, and $op(d)$ does not happen before any operation in $t[1,i]$.
\end{itemize}
...
\item[-] Assume that $q_i=(repD_i,msgs_i,<_i)$ satisfies properties $P_1$, $P_2$ and $P_3$. let us consider $q_{i+1}= (repD_{i+1},msgs_{i+1},<_{i+1})$, 
    ...
    \item[-] Else, if $q_{i+1}$ is obtained from $q_i$ by a update transition. Then we can see that $(repD_i,msgs_i,<_i) {\xrightarrow{m(a,b,r)}} (repD_{i+1},msgs_{i+1}=msgs \cup \{ (dat,r,r') \vert  r' \in RId \wedge r \neq r' \},<_{i+1} = <_i \otimes dat)$. 
        ...
        To satisfy $P_2$ and $P_3$, we need to make operations in $msgs_{i+1}$ which are ``visible to replica $r$ in $q_i$'' to be visible to $op(dat)$ in $q_{i+1}$ and still keep transitivity, which is done by $<_i \otimes dat$. 
