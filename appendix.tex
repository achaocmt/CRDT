...
Given execution $q_0 {\xrightarrow{\alpha_1}} q_1 \ldots {\xrightarrow{\alpha_k}} q_k$ of $RImp(Spec)$ and its annotated history $(O_k,\mathit{ro}_k,\mathit{vis}_k,\mathit{arb}_k)$, and assume $q_k = (O'_k,\mathit{ro}'_k,\mathit{del}'_k,\mathit{arb}'_k)$. Then, 
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $O'_k$ is the set of update operations of $O_k$.

\item[-] $\mathit{ro}'_k = \mathit{ro}_k \uparrow_{( O'_k \times O'_k )}$,

\item[-] $( \mathit{del}'_k \cdot \mathit{ro} ) \uparrow_{(O'_k \times O'_k)} = \mathit{vis}_k$,
...
Assume $RImp(Spec) = (Q,\Sigma,q_0,\rightarrow)$, $t = \alpha_1, \ldots, $ and $q_1,\ldots \in Q$, such that $q_0 {\xrightarrow{\alpha_1}} q_1 \ldots$, and for each $k$, $q_k = (O_k,\mathit{ro}_k,\mathit{del}_k,\mathit{arb}_k)$. Let $anHis(t) = (O_t,\mathit{ro}_t,\mathit{vis}_t,\mathit{arb}_t)$.
...
We need to prove that, there exists function $ctxt$, such that $\forall o \in O_t$, $ctxt(o) = (O_o,<_o,arb_o) \in Spec(lab(o))$. Assume $o$ is generated with the transition $(O_k,\mathit{ro}_k,\mathit{del}_k,\mathit{arb}_k)$ ${\xrightarrow{\alpha_{k+1}}} (O_{k+1},\mathit{ro}_{k+1},\mathit{del}_{k+1},\mathit{arb}_{k+1})$. Then, 

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] If $o$ is a query operation, then 
\end{itemize} 
