...
\noindent {\bf Example 6. The case of Distributed list}: Distributed list is a plus-minus specification, while $add$ is the ``plus method'' and $rem$ is the ``minus method''. Given $q=(O,\mathit{ro},\mathit{del},\mathit{arb})$ and a add operation $o$ where $lab(o)=add(a,pos)$, $o' \in Minus(q,o)$, if (1) $cont(o')=rem(pos,a)$, and (2) Recall that we have already check the correctness of operations of $(O,vis,arb)$ as in Example 2. When we comes to $o'$ in this process, $seq(o')[pos]=o$.
