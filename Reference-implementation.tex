...
We say that poset $ps_1$ and $ps_2$ are equivalent in specification $Spec$, if and only if for each operation $o$, $ps_1 \in Spec(o)$, if and only if $ps_2 \in Spec(o)$. We say a specification $Spec$ is a plus-minus specification, if

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] It contains a query method and two update method. Let us temporally call the query method ``read'' and two update method ``plus'' and ``minus''. (They may use different name. For example, the plus method may be called insert or put, while the minus method may be called remove). 
    
\item[-] The plus method and minus methods are distinguished as follows: given a state of $\llbracket RImp(Spec) \rrbracket$, if we do plus method at each replica, then the read method of each replica is either unchanged or returns ``more information'' (for example, an item is insert into a set); while if we do minus method at each replica, then the read method of each replica is either unchanged or returns ``less information'' (for example, an item is removed from a set). 

\item[-] Given a state $(uO,ro,del)$ of $\llbracket RImp(Spec) \rrbracket$ and a plus operation $o_p \in uO$, its direct-minus operations, denoted $dirM(o_p)$, is a set of minus operations. 
    
    A operation $o_m$ is in $dirM(o)$, if $o_p {\xrightarrow{ vis }} o_m$, and
