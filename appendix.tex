...
\item[-] We need to prove that, given path $q_0 {\xrightarrow{\alpha_1}} q_1 \ldots$ of $RImp(Spec)$, if (1) from $q_i$, $o$ is redundant and (2) $addDel(o,r)$ is not in this path, then, for $j>i$, $\alpha_1 \cdot \ldots \cdot \alpha_j \cdot addDel(o,r) \cdot \ldots$ is an execution of $RImp(Spec)$.
    ...
    This holds since (1) when we do $(add,a,\top,\_,\_)$ transitions, we always succeed, (2) when we do $(rem,\top,a,r,\_)$ transitions and add $o=(rem(\top,a),\_,\_)$, we require the existence of a $add(a,\top)$ operation $o_a$, such that $o$ is the first $rem(\top,a)$ operations that is after $o_a$ w.r.t visibility relation.  
    
    non-redundant $add(a,\top)$ operation, while the redundant $add$ and $rem$ operations do not influence this, (3) when we do $read$, the redundant $add$ operations are unseen.
