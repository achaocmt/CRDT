...
of replicas $\mathbb{R}$, where we only consider replica identifiers.
We assume that each replica contains a copy of the data type.
...
To simplify our presentation we will label operation with labels of
the form \mbox{$m(a)\Rightarrow b$} with $m \in \mathbb{M}$ and $a,b \in
\mathbb{D}$, indicating that the operation is a call to method $m$
with arguments values $a$ and the result of the operation is the value
$b$.
...
replica to receive the client's request and $i \in \mathbb{O}$ is a
unique identifier for the operation.
This definition extends naturally to the case when there are more than
one arguments.
...
is only finite.
\fxnote{GP: This example has to be developed} {\color {red}An example
  of distributed list history is shown in \figurename~\ref{fig:history,
    annotated history and operation context} (a).}

While histories provide the means to describe the behaviors of CRDTs,
they are insufficient to describe the effects of the the delivery
...
{\color {red}The annotated history for history of \figurename~\ref{fig:history, annotated history and operation context} (a) is given in \figurename~\ref{fig:history, annotated history and operation context} (b).} % with arbitration order $\mathit{add}(b,1) \cdot \mathit{add}(c,2) \cdot \mathit{add}(a,1)$.}
%\begin{itemize}
%\setlength{\itemsep}{0.5pt}
