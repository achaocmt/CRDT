...
\noindent {\bf Example 1. OR-set}: Observed-Remove Set (OR-set for short) \cite{Shapiro:2011,Bieniusa:2012} contains four methods: (1) $add(a)$ inserts $a$ into set, (2) $rem()\Rightarrow a$ remove $a$ from set, (3) $lookup(a)\Rightarrow \textit{true}$ (resp., $lookup(a)\Rightarrow \textit{false}$) represents that $a$ is in set (resp., $a$ is not in set), and $elements() \Rightarrow S$ represents that the set content is $S$. 
...
%Let $\Sigma_{\textit{ORS}} = \{ add(a,\top),rem(\top,a) \vert a \in \mathbb{D} \}$ be the set of contents of update operations. 
The specification $S_{\textit{ORS}}$ is as follows: (1) For $x=add(a)$ or $x = rem() \Rightarrow a$, $S_{\textit{ORS}}(x)$ is the set of all tuples $(O,<,arb)$ with $arb = \emptyset$, (2) For $x=lookup(a) \Rightarrow \textit{true}$ (resp., $x=lookup(a) \Rightarrow \textit{false}$), $S_{\textit{ORS}}(x)$ is the set of all tuples $(O,<,arb)$ with $arb = \emptyset$, such that the projection of operations of $a$ in $O$ contains a maximal element with operation label $add(a)$ (resp., contains no maximal element with operation label $add(a)$), and (3) $S_{\textit{ORS}}(elements() \Rightarrow S)$ is all the tuples $(O,<,arb)$ that is in $S_{\textit{ORS}}(lookup(a) \Rightarrow \textit{true})$ for each $a \in S$, and is in $S_{\textit{ORS}}(lookup(a)) \Rightarrow \textit{false}$ for each $a \notin S$. 
