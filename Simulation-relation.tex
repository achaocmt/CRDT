...
An labeled transition system (LTS, for short) is a tuple $A = (Q,\Sigma,\rightarrow,q_0)$, where $Q$ is a set of states, $\Sigma$ is an alphabet of transition labels, $\rightarrow \subseteq Q \times \Sigma \times Q$ is a transition relation and $q_0$ is the initial state. A execution of $A$ is a sequence $\alpha_1 \cdot \ldots \cdot \alpha_k \in \Sigma^*$, such that $\exists q_1,\ldots,q_k \in Q$, $q_0 {\xrightarrow{\alpha_1}} q_1 \ldots {\xrightarrow{\alpha_k}} q_k$. We say that $A$ is deterministic, if $\forall q \in Q$ and $\forall \alpha \in \Sigma$, there exists at most one $q' \in Q$, such that $q {\xrightarrow{\alpha}} q'$.

Given two LTS $A = (Q_A,\Sigma_A,\rightarrow_A,q_{0A})$ and $B = (Q_B,\Sigma_B,\rightarrow_B,q_{0B})$ with alphabet $\Sigma_A$ and $\Sigma_B$, respectively. Given functions $f_s: Q_A \times Q_B \times \Sigma_A \rightarrow \Sigma_B$ and $f_t:Q_A^* \rightarrow Q_B$, we define our simulation and refinement relations as follows:
...
\item[-] If $f_s(q_a,q_b,\alpha) \neq \epsilon$, then $q_b {\xrightarrow{f_s(q_a,q_b,\alpha)}}_A q'_b \wedge (q'_a,q'_b) \in R$.

\item[-] If $f_s(q_a,q_b,\alpha) = \epsilon$, then $(q'_a,q_b) \in R$.
\end{itemize}
\end{definition}

\begin{definition}[$f_t$-refinement]
\label{definition:ft refinement}
$B$ $f_t$-refines $A$, if for each execution $t_A = \alpha_1 \cdot \ldots \cdot \alpha_k$ of $A$, $t_B = f_t(\alpha_1) \cdot \ldots \cdot f_t(\alpha_1 \cdot \ldots \cdot \alpha_k)$ is an execution of $B$.
\end{definition}

If $B$ is deterministic, then we defined a property $P_{(f_s,f_t)}$ as follows: For each transitions $q_{0A} {\xrightarrow{\alpha_1}} q_{1A} \ldots {\xrightarrow{\alpha_k}} q_{kA}$ of $A$, $\forall 1 \leq i < k$, $f_t(\alpha_1 \cdot \ldots \cdot \alpha_i \cdot \alpha_{i+1}) = f_s(q_{iA},q_{iB},\alpha_{i+1})$, where $q_{iB}$ is obtained from $q_{0B}$ by doing $f_t(\alpha_1) \cdot \ldots \cdot f_t(\alpha_1 \cdot \ldots \cdot \alpha_i)$ transitions. The following theorem states that when $P_{(f_s,f_t)}$ holds, $f_s$-simulation is equivalent to $f_t$-refinement. Its proof can be found in Appendix \ref{sec:appendix definitions and proofs of section simulation and refinement} and is similar to that in \cite{Abadi:1991,Lynch:1995}.  

\begin{theorem}
\label{theorem:equivalence of our simulation and refinement} 
Given LTS $A$ and deterministic $B$, and functions $f_s,f_t$, such that $P_{(f_s,f_t)}$ holds. Then, B $f_t$-refines $A$, if and only if there exists a $f_s$-simulation relation between $A$ and $B$. 
\end{theorem}
