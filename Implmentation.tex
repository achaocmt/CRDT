...
Each replica also manages a set of ``tombstones'' $T$, which are identifiers of values that were removed from the array. We define an order $<_{\mathit{ts}}$ between timestamps by $(x_1,r_1) <_{\mathit{ts}} (x_2,r_2)$ iff $x_1 < x_2 \vee (x_1 = x_2 \wedge r_1 < r_2)$. Let $\mathit{ts}(N)$ be the set of timestamps in $N$ and let $\mathit{trav}(N,T)$ be the sequences obtained by traversing $N$ in prefix order (children are visited in decreasing timestamp order) and keeping only identifiers of nodes that are not in $T$.
