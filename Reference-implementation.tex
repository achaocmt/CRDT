...
\item[-] {\color {red}Let $ctxt$ be a function that takes $(O,ro,del,arb) \in Q$ and $o=(\_,r,\_) \notin O$ as arguments, and returns a set of operation contexts. Each element in this set is a operation context of $o$ for some annotated history $(O_1,ro_1,vis_1,arb_1)$ satisfies that
    ...
    \begin{itemize}
    \setlength{\itemsep}{0.5pt}
    \item[-] $O_1 = visTo(O,r,vis) \cup \{ o \}$. Here $visTo(O,r,vis) = \{ vis^{-1}(o') \vert o'=(\_,r,\_) \in O \} \cup (vis^{-1}(r))$.
    ...
    \item[-] $arb_1 \subseteq O_1 \times O_1$ is obtained from $arb$ by projection into $visTo(O,r,vis)$, and then add operation $o$ and possibly some relation between $O$ and $\{ o \}$.
    \end{itemize}}
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Given $q = (O,ro,del,arb)$ and $o =(\_,r,\_)$, $ctxt(q,o) = ( O_1 ,<_1, arb_1 )$, where $O_1 = visTo(O,r,vis)$, $<_1 = vis \uparrow_{(O_1 \times O_1)}$, and $arb_1$ is as follows: (1) if $lab(o)=add(\_)$, then $arb_1$ is a total order generated by adding $o$ into some place of $arb$, (2) otherwise, $arb_1 = arb$.
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] {\color {red}$O$ is the union of two set. One set is the update operations generated during $e$, and the other set is one operation for each query transitions (Assume each $(m,a,b,r,arb)$ represents a $(m(a)\Rightarrow b,r,i)$ operation for some $i$).} 
...
$\begin{array}{l c} \bigfrac{o \in O, o \ is \ minimal \ w.r.t \ <_{hb} in \ \{ o' \vert o' \in O \wedge o' \notin visTo(O,r,vis) \}} {(O,ro,del,arb) {\xrightarrow{addDel(o,r)}} (O,ro,del \cup \{ (o,r) \},arb)} \end{array}$
