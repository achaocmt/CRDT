...
\item[-] $C_{\mathit{mhv}}$: Given $m_1,m_2 \in \mathit{msgs}$ and assume that their operations are $x_1$ and $x_2$, respectively. If $x_1$ and $x_2$ are both non-redundant, then, $(m_1,m_2) \in \mathit{msghb}$, if and only if $f(x_1)$ happens before $f(x_2)$, and $(m_1,r) \in \mathit{msghb}$, if and only if $(f(x_1),r) \in \mathit{del}$. 
\end{itemize}
