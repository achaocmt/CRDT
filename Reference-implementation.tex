...
\item[-] {\color {red}Let $ctxt$ be a function that takes $(O,ro,del,arb) \in Q$ and $o=(\_,r,\_) \notin O$ as arguments, and returns a operation contexts. Such operation context is a operation context of $o$ for some annotated history $(O_1,ro_1,vis_1,arb_1)$ satisfies that
...
%It contains three kinds of transitions: adding a deliver relation, doing a query operation, and doing an update operation.
    ...
    \item[-] Doing a query operation: Let $m$ be a query method. For $\ell = m(a) \Rightarrow b$,
     ...
     $\begin{array}{l c} \bigfrac{ o=(\ell,r,\_) \notin O, \exists O', ctxt((O,ro,del,arb),o) = (O',\_,arb \uparrow_{ (Q' \times Q') })  \in Spec(\ell)} {(O,ro,del,arb) {\xrightarrow{m(a,b,r,arb)}} (O,ro,del,arb)}  \end{array}$
     ...
     $\begin{array}{l c} \bigfrac{o = (\ell,r,\_) \notin O, \exists O', ctxt((O,ro,del,arb),o) = (O',\_,arb' \uparrow_{(O' \cup \{ o \})}) \in Spec(\ell)} {(O,ro,del,arb) {\xrightarrow{m(a,b,r,arb')}} (O \cup \{ o \},ro \oplus o ,del \oplus o,arb')}  \end{array}$
