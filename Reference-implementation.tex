...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] It contains two kinds of update operations. The first kind is ``add operation'', which intuitively add some item into ``abstract state''. The second kind is ``minus operation'', which intuitively remove some item from ``abstract state''. 
